  ...
  return (
    <div className='relative flex flex-col justify-center items-center bg-gray-900 rounded-lg w-full h-screen overflow-hidden'>
      ...
      <div className={cn('top-4 left-4 z-50 absolute')}>
        <button
          onClick={toggleMenu}
          className='fixed p-4 rounded-full text-white focus:outline-none'
        >
          {isMenuOpen ? (
      ...
      >
        <ul className='space-y-6 font-medium text-center text-lg'>
          <li>
            <a href='/'>HOME</a>
          </li>
          <li>
      ...
      {/* Main Content */}
      <section className='absolute flex flex-row justify-between items-center ml-0 md:ml-12 px-8 sm:px-12 lg:px-32 md:xp-20 min-h-screen'>
        {/* Right Section: Hero Text */}
        <div className='flex-1 mt-8 ml-0 md:ml-12 text-center md:text-left'>
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.8 }}
            className='opacity-0 transform scale-90'
          >
          ...
          <motion.h1
            ...
            className='font-bold text-4xl text-black md:text-5xl'
          >
            <span className='text-teal-500'>Web Developer</span>
          </motion.h1>

          <div className='relative z-50 flex justify-center md:justify-start space-x-4 mt-8'>
            <motion.a
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
